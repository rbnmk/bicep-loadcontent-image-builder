...
@description('URI of the sysprepped image vhd file to be used to create the session host VMs. For example, https://rdsstorage.blob.coore.windows.net/vhds/sessionhostimage.vhd')
...
resource vm_bginfo 'Microsoft.Compute/virtualMachines/extensions@2018-10-01' = [for i in range(0, rdshNumberOfInstances): {
  name: '${rdshPrefix}${(i + vmInitialNumber)}/BGInfo'
  location: location
  properties: {
    publisher: 'Microsoft.Compute'
    type: 'BGInfo'
...
resource vm_autoshutdown 'Microsoft.DevTestLab/schedules@2018-09-15' = [for i in range(0, rdshNumberOfInstances): {
  name: 'shutdown-computevm-${rdshPrefix}${(i + vmInitialNumber)}'
  location: location
  tags: {
    description: 'Shutdown schedule for ${rdshPrefix}${(i + vmInitialNumber)}'
  }
  properties: {
    status: 'Enabled'
    ...
    targetResourceId: resourceId('Microsoft.Compute/virtualMachines', '${rdshPrefix}${(i + vmInitialNumber)}')
  }
  dependsOn: [
    vm
  ]
